...
### 1. 準備：Cursorウィンドウを複数開く（重要）

**⚠️ 注意: 同じウィンドウで「チャットタブ」を複数開くのはNGです。**
タブを切り替えてもGitブランチは自動で切り替わらないため、作業内容が混ざる事故が起きます。

必ず以下のショートカットで **「新しいウィンドウ」** を開き、それぞれのウィンドウで別のブランチを操作してください。
...
| ウィンドウA             | ウィンドウB              |
| ----------------------- | ------------------------ |
| `feature/hero` ブランチ | `feature/about` ブランチ |
| ヒーローセクション担当  | Aboutセクション担当      |
...
「このスタイル、どのSCSSファイルに書いてあるんだっけ？」と迷ったら、ブラウザの検証ツールを活用しましょう。

1. Google Chromeで要素を右クリック → **「検証」**
2. Stylesタブを見る
3. `style.css` ではなく **`_header.scss:25`** のように、元のファイル名が表示されます（ソースマップ機能）。
4. クリックするとファイルの中身が確認できます。
